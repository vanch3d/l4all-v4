...
return'';}();}
if(typeof Zapatec=='undefined'){Zapatec=function(){};}
Zapatec.ErrorHandler={};Zapatec.ErrorHandler.setup=function(window,report_url){this.window=window;this.report_url=report_url;this.window.onerror=Zapatec.ErrorHandler.handleError;}
Zapatec.ErrorHandler.handleError=function(desc,path,line){if(Zapatec.ErrorHandler.err_stack==undefined)Zapatec.ErrorHandler.err_stack=[];Zapatec.ErrorHandler.err_stack.push([desc,path,line]);return false;}
Zapatec.ErrorHandler.getErrStack=function(){return Zapatec.ErrorHandler.err_stack;}
...
return r;};Zapatec.Utils.getElementOffset=function(oEl){var iLeft=iTop=iWidth=iHeight=0;if(oEl.getBoundingClientRect){var oRect=oEl.getBoundingClientRect();iLeft=oRect.left;iTop=oRect.top;iWidth=oRect.right-iLeft;iHeight=oRect.bottom-iTop;iLeft+=Zapatec.Utils.getPageScrollX()-2;iTop+=Zapatec.Utils.getPageScrollY()-2;}else{iWidth=oEl.offsetWidth;iHeight=oEl.offsetHeight;var sPos=Zapatec.Utils.getStyleProperty(oEl,'position');if(sPos=='fixed'){iLeft=oEl.offsetLeft+Zapatec.Utils.getPageScrollX();iTop=oEl.offsetTop+Zapatec.Utils.getPageScrollY();}else if(sPos=='absolute'){while(oEl.offsetParent){iLeft+=oEl.offsetLeft;iTop+=oEl.offsetTop;oEl=oEl.offsetParent;iLeft-=oEl.scrollLeft;iTop-=oEl.scrollTop;}}else{var oP=oEl;while(oP.offsetParent){iLeft+=oP.offsetLeft;iTop+=oP.offsetTop;oP=oP.offsetParent;}
oP=oEl;while(oP.parentNode){oP=oP.parentNode;var sTag=oP.tagName;if(sTag){sTag=sTag.toLowerCase();if(sTag!='body'&&sTag!='html'){iLeft-=oP.scrollLeft;iTop-=oP.scrollTop;}}}}}
...
el.className=ar.join(" ");};Zapatec.Utils.addClass=function(el,className){Zapatec.Utils.removeClass(el,className);el.className+=" "+className;};Zapatec.Utils.getElement=function(ev){if(Zapatec.is_ie){return window.event.srcElement;}else{return ev.currentTarget;}};Zapatec.Utils.getTargetElement=function(ev){if(Zapatec.is_ie){return window.event.srcElement;}else{return ev.target;}};Zapatec.Utils.getMousePos=function(objEvent){objEvent||(objEvent=window.event);var objPos={pageX:0,pageY:0,clientX:0,clientY:0};if(objEvent){var boolIsPageX=(typeof objEvent.pageX!='undefined');var boolIsClientX=(typeof objEvent.clientX!='undefined');if(boolIsPageX||boolIsClientX){if(boolIsPageX){objPos.pageX=objEvent.pageX;objPos.pageY=objEvent.pageY;}else{objPos.pageX=objEvent.clientX+Zapatec.Utils.getPageScrollX();objPos.pageY=objEvent.clientY+Zapatec.Utils.getPageScrollY();}
if(boolIsClientX){objPos.clientX=objEvent.clientX;objPos.clientY=objEvent.clientY;}else{objPos.clientX=objEvent.pageX-Zapatec.Utils.getPageScrollX();objPos.clientY=objEvent.pageY-Zapatec.Utils.getPageScrollY();}}}
return objPos;};Zapatec.Utils.stopEvent=function(ev){ev||(ev=window.event);if(ev){if(Zapatec.is_ie){ev.cancelBubble=true;ev.returnValue=false;}else{ev.preventDefault();ev.stopPropagation();}}
...
('beforeEnd','<iframe id="'+id+'" scrolling="no" frameborder="0" '+'style="z-index:0;position:absolute;visibility:hidden;'+filter+'border:0;top:0;left:0;width:0;height:0" '+'src="about:blank"></iframe>');f=document.getElementById(id);}
...
if(Zapatec.Utils.getStyleProperty(lnk,"color")==Zapatec.Utils.getStyleProperty(parent,"background-color")){return false;}
...
alert(strMsg)
return false;}
Zapatec.Utils.clone=function(objSource){var objClone;if(!objSource&&typeof objSource=='object'){return null;}else if(typeof(objSource)=='undefined'){return objClone;}
if((objSource instanceof String)||(objSource instanceof Number)||(objSource instanceof Boolean)){objClone=new objSource.constructor(objSource.valueOf());}else{objClone=new objSource.constructor();}
for(var strProperty in objSource){if(typeof objSource[strProperty]=='object'){objClone[strProperty]=Zapatec.Utils.clone(objSource[strProperty],true);}else{objClone[strProperty]=objSource[strProperty];}}
return objClone;};Zapatec.is_opera=/opera/i.test(navigator.userAgent);Zapatec.is_ie=(/msie/i.test(navigator.userAgent)&&!Zapatec.is_opera);Zapatec.is_ie5=(Zapatec.is_ie&&/msie 5\.0/i.test(navigator.userAgent));Zapatec.is_ie7=(Zapatec.is_ie&&/msie 7\.0/i.test(navigator.userAgent));Zapatec.is_mac_ie=(/msie.*mac/i.test(navigator.userAgent)&&!Zapatec.is_opera);Zapatec.is_khtml=/Konqueror|Safari|KHTML/i.test(navigator.userAgent);Zapatec.is_konqueror=/Konqueror/i.test(navigator.userAgent);Zapatec.is_gecko=/Gecko/i.test(navigator.userAgent);if(!Object.prototype.hasOwnProperty){Object.prototype.hasOwnProperty=function(strProperty){try{var objPrototype=this.constructor.prototype;while(objPrototype){if(objPrototype[strProperty]==this[strProperty]){return false;}
...
tmp=tmp[keys[ii]];}}
Zapatec.implement=function(classOrObject,interfaceObj){if(typeof classOrObject=="function"){classOrObject=classOrObject.prototype;}
if(!classOrObject||typeof classOrObject!="object"){return false;}
if(!Zapatec.isArray(classOrObject.interfaces)){classOrObject.interfaces=[interfaceObj];}else if(Zapatec.Array.indexOf(classOrObject.interfaces,interfaceObj)==-1){classOrObject.interfaces.push(interfaceObj);}else{return true;}
for(var iProp in interfaceObj){classOrObject[iProp]=interfaceObj[iProp];}
classOrObject.hasInterface=function(interfaceObj){if(Zapatec.Array.indexOf(this.interfaces,interfaceObj)!=-1){return true;}
return false;}
...
var sPath='';if(sImage.indexOf('/')<0){sPath=Zapatec.Transport.getPath('transport.js');}
...
(typeof sId=='string'?'" id="'+sId:'')+'"></script>');Zapatec.Transport.loadedJS[sAbsUrl]=true;};Zapatec.include=Zapatec.Transport.include;Zapatec.Transport.includeJS=function(sSrc,sId){if(Zapatec.doNotInclude){return;}
...
oContr.appendChild(oScript);};Zapatec.Transport.loadJS=function(oArg){if(!(oArg instanceof Object)){return;}
if(typeof oArg.async=='undefined'){oArg.async=true;}
...
if(!this.events[sEvent]){this.events[sEvent]={listeners:[]};}
...
var aListeners=Zapatec.EventDriven.events[sEvent].listeners;for(var iListener=0;iListener<aListeners.length;iListener++){var aArgs=[].slice.call(arguments,1);aListeners[iListener].apply(aListeners[iListener],aArgs);}};if(typeof Zapatec=='undefined'){Zapatec=function(){};}
...
if(typeof(Zapatec.Effects)=='undefined'){var self=this;Zapatec.Transport.loadJS({url:Zapatec.Widget.path+'../../zpeffects/src/effects.js',onLoad:function(){self.showHideContainer(effects,animSpeed,onFinish,show);}});return false;}
if(animSpeed==null&&isNaN(parseInt(animSpeed))){animSpeed=5;}
if(effects==null||effects.length==0){if(show){this.container.style.display=this.originalContainerDisplay;this.originalContainerDisplay=null;}else{this.originalContainerDisplay=this.container.style.display;this.container.style.display='none';}
