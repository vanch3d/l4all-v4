...
Timeline.DurationEventPainter.prototype.paint = function() {
    ...
    //if (this._timeline.isHorizontal()) {
        ...
        var createInstantDiv = function(evt, startPixel, endPixel, streamOffset, highlightIndex, highlightOffset, highlightWidth) {
			...
			var lengthEvt = Math.max(endPixel - startPixel, 1);
            ...
            if (evt.isImprecise()) { // imprecise time
                ...
                divImprecise.style.width = lengthEvt + "px";
                divImprecise.style.background = (background==null)?eventTheme.instant.impreciseColor:background;
            ...
            if (showText) {
                ...
                //alert("label: " + eventTheme.label.width + " length: " + lengthEvt);
                var calcw = Math.min(eventTheme.label.width,lengthEvt-realign);
                calcw = Math.max(lengthEvt-realign,calcw);
                //div.style.width = calcw + "px";
                //div.style.width = Math.min(eventTheme.label.width,lengthEvt-realign);
                //div.style.width = Math.max(lengthEvt-realign,div.style.width) + "px";
